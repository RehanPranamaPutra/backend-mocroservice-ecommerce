...
// --- Helper Response ---
const success = (res, message, data = null) => res.status(200).json({ success: true, message, data });
...
app.post('/login', async (req, res) => {
    try {
        ...
        success(res, 'Login successful', { token, role: user.role });
    } catch (err) {
...
app.put('/users/:id', authenticate, async (req, res) => {
    try {
        ...
        await user.save();
        success(res, 'User updated successfully', { id: user.id, name: user.name, email: user.email });
    } catch (err) {
        errorResponse(res, 500, err.message);
    }
});
